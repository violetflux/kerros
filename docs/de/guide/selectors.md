...
React-Elemente und Portale werden automatisch als atomare Werte erkannt. Standardcontainer aus `useRef()` und `createRef()` können unter React 17, 18 und 19 direkt zurückgegeben werden. Verwende `ref(value)` nur für Proxy-inkompatible Drittanbieterobjekte oder wenn strikte Objektidentität erhalten bleiben muss. Interne Mutationen atomarer Werte sind nicht reaktiv; veröffentliche eine neue Referenz, wenn eine Änderung rendern soll.

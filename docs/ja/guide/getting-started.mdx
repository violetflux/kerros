import { PackageManagerTabs } from '@rspress/core/theme'
import CounterDemo from '../../_components/CounterDemo'

# クイックスタート

## インストール

<PackageManagerTabs command={{
  npm: 'npm install @violetflux/kerros',
  pnpm: 'pnpm add @violetflux/kerros',
  yarn: 'yarn add @violetflux/kerros',
  bun: 'bun add @violetflux/kerros',
}} />

Kerros は npm、pnpm、Yarn、Bun で利用でき、React 17、18、19 をサポートします。

## Store を作成する

任意の custom Hook を `createStore` でラップすると、コンポーネント間で状態を共有できます。

```tsx
import { createStore } from '@violetflux/kerros'
import { useState } from 'react'

interface Task {
  id: string
  title: string
}

function useTaskModel() {
  const [tasks, setTasks] = useState<Task[]>([])

  const addTask = (task: Task) => {
    setTasks(v => [...v, task])
  }

  const finishTask = (taskId: string) => {
    setTasks(v => v.filter(task => task.id !== taskId))
  }

  return { tasks, addTask, finishTask }
}

export const [useTask, TaskProvider, getTask] = createStore(useTaskModel)
```

`createStore` は、コンポーネントが使う Hook、対応する Provider、React 外からコミット済みインスタンスを読む命令的 getter を返します。Store の中では `useState`、`useReducer`、Context、SDK Hook、独自 Hook をそのまま使えます。

initializer は `useTaskModel` のようなトップレベルの名前付き Hook にしてください。匿名 initializer も動作しますが、React Compiler の `infer` モードでは Hook として自動コンパイルされません。

## Provider を配置する

`useTask` は `TaskProvider` の子孫でのみ呼び出せます。

```tsx
function App() {
  return (
    <TaskProvider>
      <Header />
      <TaskList />
    </TaskProvider>
  )
}
```

Provider の外で呼び出すと、Kerros は明確なエラーを投げます。

## コンポーネントで Store を使う

`useTask()` を引数なしで呼び、必要なフィールドを直ちに分割代入します。自動追跡がデフォルトです。

```tsx
function TaskList() {
  const { tasks, finishTask } = useTask()

  return (
    <ul>
      {tasks.map(task => (
        <li key={task.id}>
          {task.title}
          <button onClick={() => finishTask(task.id)}>完了</button>
        </li>
      ))}
    </ul>
  )
}
```

Kerros はレンダー中に読み取ったフィールドを追跡します。未読フィールドの変更では `TaskList` は再レンダーされません。明示的 selector は派生値や計測済みホットスポットでのみ使います。

## 複数の Store インスタンス

Provider を配置するたびに、独立した Store が作られます。

```tsx
<TaskProvider>
  <h2>個人タスク</h2>
  <TaskList />
</TaskProvider>

<TaskProvider>
  <h2>チームタスク</h2>
  <TaskList />
</TaskProvider>
```

各 `TaskList` は最も近い Provider を自動的に読み、データは互いに影響しません。

## Store 間の依存

ある Store から別の Store を直接呼び出せます。

```tsx
function useTaskModel() {
  const { user } = useAccount()
  const [tasks, setTasks] = useState<Task[]>([])

  const addTask = (title: string) => {
    if (!user)
      return

    setTasks(v => [...v, {
      id: crypto.randomUUID(),
      title,
      assigneeId: user.id,
    }])
  }

  return { tasks, addTask }
}

export const [useTask, TaskProvider] = createStore(useTaskModel)
```

依存される Provider を外側に配置し、依存関係を一方向に保ちます。

```tsx
<AccountProvider>
  <TaskProvider>
    <App />
  </TaskProvider>
</AccountProvider>
```

## Provider に props を渡す

Provider の props は Store Hook の引数になります。

```tsx
function useCounterModel({ initialCount }: { initialCount: number }) {
  const [count, setCount] = useState(initialCount)
  return { count, setCount }
}

const [useCounter, CounterProvider] = createStore(useCounterModel)
```

```tsx
<CounterProvider initialCount={42}>
  <Counter />
</CounterProvider>
```

## 試してみる

このカウンターも同じ `createStore + Provider + 自動追跡` API を使っています。

<CounterDemo />

次は [Selector](./selectors) と [Store の合成](./composition) を参照してください。

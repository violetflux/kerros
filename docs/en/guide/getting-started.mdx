...
`createStore` returns the Hook used by components, its Provider, and an imperative getter for committed instances outside React. Name them after the domain, such as `useTask`, `TaskProvider`, and `getTask`; repeating `Store` in their names is optional.
